...
abstract contract Helix2Polycules {
    ...
    event RehookedConfig(bytes32 indexed polyculehash, address config, uint8 rule);
    event UnhookedConfig(bytes32 indexed polyculehash, address config);
    event RehookedAnion(bytes32 indexed polyculehash, bytes32 anion, uint8 rule);
    ...
    event NewSecureFlag(bytes32 indexed polyculehash, bytes32 anion, bool secure);
    ...
    */
    ...
    modifier isCationOrController(bytes32 polyculehash) {
        ...
        require(
            _cation == msg.sender || Operators[_cation][msg.sender] || msg.sender == Polycules[polyculehash]._controller, 
     ...
     */
    function addAnion(bytes32 polyculehash, bytes32 _anion, address config, uint8 rule) external isCationOrController(polyculehash) {
        require(
            isNotDuplicateAnion(polyculehash, _anion),
            "ANION_EXISTS"
        );
        require(
            isNotDuplicateHook(polyculehash, config),
            "HOOK_EXISTS"
        );
        require(
    ...
    function setAnions(
        ...
        isCationOrController(polyculehash) 
    {
        require(
            isLegalMap(_anion, _config), 
            'BAD_MAP'
        );
     ...
     */
    function popAnion(bytes32 polyculehash, bytes32 __anion) external isCationOrController(polyculehash) {
     ...
     */
    function switchSecure(bytes32 polyculehash, bytes32 __anion) external isCationOrController(polyculehash) {
        bytes32[] memory _anion = Polycules[polyculehash]._anion;
        if (__anion.existsIn(_anion)) {
            uint index = __anion.findIn(_anion);
     ...
     */
    function setSecure(bytes32 polyculehash, bool[] memory _secure) external isCationOrController(polyculehash) {
     ...
     */
    function setExpiry(bytes32 polyculehash, uint _expiry) external payable isCationOrController(polyculehash) {
        require(
            _expiry > Polycules[polyculehash]._expiry,
            "BAD_EXPIRY"
        );
     ...
     */
    function cation(bytes32 polyculehash) public view isNotExpired(polyculehash) returns (bytes32) {
        bytes32 __cation = Polycules[polyculehash]._cation;
        address _cation = NAMES.owner(__cation);
     ...
     */
    function controller(bytes32 polyculehash) public view isNotExpired(polyculehash) returns (address) {
     ...
     */
    function secure(bytes32 polyculehash) public view isNotExpired(polyculehash) returns (bool[] memory) {
     ...
     */
    function hooks(bytes32 polyculehash) public view isNotExpired(polyculehash) returns (address[] memory, uint8[] memory) {
        address[] memory _hooks = Polycules[polyculehash]._hooks;
     ...
     */
    function anion(bytes32 polyculehash) public view isNotExpired(polyculehash) returns (bytes32[] memory) {
        bytes32[] memory _anion = Polycules[polyculehash]._anion;
        return _anion;
    }
     ...
     */
    function resolver(bytes32 polyculehash) public view isNotExpired(polyculehash) returns (address) {
