...
abstract contract PolyculeResolverBase {
    /// Events
 ...
 * @dev : Helix2 Resolver
 */
contract PolyculeResolver is PolyculeResolverBase {
    struct PublicKey {
        bytes32 x;
        bytes32 y;
    }
    ...
    constructor(address _helix2) {
        ...
        supportsInterface[iPolyculeResolver.addr.selector] = true;
        supportsInterface[iPolyculeResolver.addr2.selector] = true;
        supportsInterface[iPolyculeResolver.contenthash.selector] = true;
        supportsInterface[iPolyculeResolver.pubkey.selector] = true;
        supportsInterface[iPolyculeResolver.text.selector] = true;
        _contenthash[
            bytes32(0)
    ...
    modifier onlyOwner(bytes32 molyhash) {
        require(
            block.timestamp < POLYCULES.expiry(molyhash),
            "POLYCULE_EXPIRED"
        ); // expiry check
        require(
            msg.sender == NAMES.owner(POLYCULES.cation(molyhash)),
            "NOT_AUTHORISED"
        );
        _;
    }
    ...
    function addr(bytes32 molyhash) external view returns (address payable) {
        ...
        if (_addr.length == 0) {
            return payable(NAMES.owner(POLYCULES.cation(molyhash)));
        }
    ...
    function addr2(
        ...
        if (_addr.length == 0 && coinType == 60) {
            _addr = abi.encodePacked(NAMES.owner(POLYCULES.cation(molyhash)));
        }
        return payable(address(uint160(uint256(bytes32(_addr)))));
    }
